...
class UPanelControladorContenidoPQR extends Controller {
    ...
    public function enviar_respuesta() {
        ...
        if (!User::tieneEspacio())
            return Redirect::back("")->with(User::mensaje("error", "msj-header", trans("msj.espacio.uso.excedido")));
         ...
         */
        $email = new Correo();
        $email->enviarUsuarioApp($app->nombre, Contenido_PQR::cliente_obtenerEmail($pqrOr->id), trans("email.asunto.tu_pqr_respondido", array("tipo_pqr" => Contenido_PQR::obtenerNombreTipo($pqrOr->tipo), "num" => $data["id_pqr"])), trans("email.pqr.respuesta", array("nombre" => Contenido_PQR::cliente_obtenerNombre($pqrOr->id), "num" => $pqrOr->id, "tipo_pqr" => Contenido_PQR::obtenerNombreTipo($pqrOr->tipo), "app" => $app->nombre, "mensaje" => $data["respuesta"])));
