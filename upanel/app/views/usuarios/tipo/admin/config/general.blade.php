...
$moneda_select = Instancia::obtenerValorMetadato(ConfigInstancia::info_moneda);
?>
        ...
        <div class="panel-body">
            ...
            <span  href="#" class="tooltip-left" rel="tooltip" id='{{ConfigInstancia::visual_logo}}-content' title="{{trans('otros.extensiones_permitidas')}}: png, jpeg. Max 500Kb"> 
                ...
                <form id="form-config" action="{{URL::to("config/post/guardar")}}" method="POST">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" name="{{ ConfigInstancia::periodoPrueba_numero_dias; }}" value="{{ Instancia::obtenerValorMetadato(ConfigInstancia::periodoPrueba_numero_dias); }}"/>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">{{trans("config.general.seccion.moneda_principal")}}</div>
                        <div class="panel-body">
                            <div class="col-lg-6">
                                <label> {{trans("config.general.seccion.moneda_principal.op.moneda")}} @include("interfaz/util/tooltip-ayuda",array("descripcion"=>trans('config.general.seccion.moneda_principal.op.moneda.ayuda')))</label>
                            </div>
                            <div class="col-lg-6">
                                <select class="form-control" id="{{ConfigInstancia::info_moneda}}" name="{{ConfigInstancia::info_moneda}}">
                                    @foreach($monedas as $index => $valor)
                                    <option value="{{$index}}" @if($moneda_select==$index) selected @endif>{{$valor}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
